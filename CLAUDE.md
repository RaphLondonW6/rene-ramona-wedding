...
- **D1 database `wedding-posts`** (binding `DB`) — `posts` table (metadata incl. email, never exposed publicly, and optional `caption` capped at 80 chars) + `upload_log` (per-IP rate limiting). Schema: `schema.sql`. Migrations live in `migrations/` — run new ones with `npx wrangler d1 execute wedding-posts --remote --file=migrations/00X_name.sql`.
