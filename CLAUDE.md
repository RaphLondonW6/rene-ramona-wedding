...
- Hero section with a **scrolling photo gallery wall** — a single CSS Grid (`display:grid; grid-template-rows:repeat(3,1fr); grid-auto-flow:column; grid-auto-columns:30vh`) showing ~4 columns of photos at a time, scrolling slowly left-to-right as one unit (90s loop). Dark overlay on top. Photos live in `public/stag/photos/` numbered `1.jpg` → `25.jpg` (photo 4 is `4.png`). The grid contains 60 images (photos 1–25 + filler to reach 30, duplicated for a seamless loop via `@keyframes scroll-right: translateX(-50%) → translateX(0)`). Original photos sourced from `C:\Users\RSCHAEFFER\OneDrive - ACCOR\Accor\CLAUDE\RENE_Ramona_Wedding\Pictures\Stag` — HEIC files were converted to JPG before copying. **To replace or add photos:** rename the new files numerically, copy to `public/stag/photos/`, and update the `<img>` tags in the `.photo-grid` div inside the hero.
