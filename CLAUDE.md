...
- Hero section with a **scrolling photo gallery wall** — 3 rows of real stag photos scrolling left-to-right at different speeds (55s / 75s / 42s), dark overlay on top. Photos live in `public/stag/photos/` numbered `1.jpg` → `25.jpg` (photo 4 is `4.png`). Each row duplicates the photo set for a seamless infinite loop (`@keyframes scroll-right: translateX(-50%) → translateX(0)`). Original photos sourced from `C:\Users\RSCHAEFFER\OneDrive - ACCOR\Accor\CLAUDE\RENE_Ramona_Wedding\Pictures\Stag` — HEIC files were converted to JPG before copying. **To replace or add photos:** rename the new files numerically, copy to `public/stag/photos/`, and update the `<img>` tags in the three `.photo-row` divs inside the hero.
